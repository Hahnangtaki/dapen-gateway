...
export const ProvinceMySuffix = (props: IProvinceMySuffixProps) => {
  useEffect(() => {
    props.getEntities();
  }, []);

  const { provinceList, match, loading } = props;
  return (
    <div>
      <h2 id="province-my-suffix-heading">
        <Translate contentKey="dapenApp.province.home.title">Provinces</Translate>
        <Link to={`${match.url}/new`} className="btn btn-primary float-right jh-create-entity" id="jh-create-entity">
          <FontAwesomeIcon icon="plus" />
          &nbsp;
          <Translate contentKey="dapenApp.province.home.createLabel">Create new Province</Translate>
        </Link>
      </h2>
        ...
        {provinceList && provinceList.length > 0 ? (
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="dapenApp.province.home.notFound">No Provinces found</Translate>
            </div>
          )
        )}
      </div>
...
export default connect(mapStateToProps, mapDispatchToProps)(ProvinceMySuffix);
