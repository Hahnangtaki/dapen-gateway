...
import CountryComponentsPage, { CountryDeleteDialog } from './country-my-suffix.page-object';
import CountryUpdatePage from './country-my-suffix-update.page-object';
import {
  waitUntilDisplayed,
...
describe('Country e2e test', () => {
  ...
  it('should load Countries', async () => {
    await navBarPage.getEntityPage('country-my-suffix');
  ...
  it('should load create Country page', async () => {
    ...
    expect(await countryUpdatePage.getPageTitle().getAttribute('id')).to.match(/dapenApp.country.home.createOrEditLabel/);
    await countryUpdatePage.cancel();
  });

  it('should create and save Countries', async () => {
    await countryComponentsPage.createButton.click();
    await countryUpdatePage.setCountryCodeDigitsInput('countryCodeDigits');
    expect(await countryUpdatePage.getCountryCodeDigitsInput()).to.match(/countryCodeDigits/);
    ...
    await countryUpdatePage.setNationalityInput('nationality');
    expect(await countryUpdatePage.getNationalityInput()).to.match(/nationality/);
  ...
  it('should delete last Country', async () => {
    ...
    expect(await countryDeleteDialog.getDialogTitle().getAttribute('id')).to.match(/dapenApp.country.delete.question/);
